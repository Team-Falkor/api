...
import type { Server as ElysiaServer } from "elysia/dist/universal/server";
import ms from "ms";
import type { RateLimitOptions, RateLimitStore } from "../../../@types/plugins";
...
export const rateLimitPlugin = (userOptions: RateLimitOptions = {}) => {
	...
	return new Elysia().onBeforeHandle(
		...
		({ request, set, path, server, error }) => {
			...
			let tierMax: number = options.max ?? (DEFAULT_OPTIONS.max as number);
			let tierWindow: number =
				options.windowMs ?? (DEFAULT_OPTIONS.windowMs as number);
